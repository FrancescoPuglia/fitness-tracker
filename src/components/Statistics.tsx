...
export default function Statistics() {
  ...
  const loadStatistics = useCallback(() => {
    ...
    while (current <= endDate) {
      ...
      const completedMeals = weekDiets.reduce(
        (sum, d) => sum + d.meals.filter((m) => m.completed).length,
        0
      );
      ...
      const totalCalories = weekDiets.reduce(
        (sum, d) =>
          sum +
          d.meals
            .filter((m) => m.completed)
            .reduce((mealSum, m) => mealSum + (m.calories || 0), 0),
        0
      );
  ...
  const calculateOverallStats = () => {
    ...
    const completedMeals = diets.reduce(
      (sum, d) => sum + d.meals.filter((m) => m.completed).length,
      0
    );

    const avgDietCompliance =
      totalMeals > 0 ? (completedMeals / totalMeals) * 100 : 0;
    ...
    const avgWorkoutTime =
      totalWorkoutDays > 0 ? totalWorkoutTime / totalWorkoutDays : 0;
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <select
            ...
            onChange={(e) =>
              setSelectedPeriod(e.target.value as '30' | '90' | '365')
            }
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">
              {overallStats.avgDietCompliance}%
            </div>
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-orange-600">
              {overallStats.avgWorkoutTime}min
            </div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <div className="font-medium">
                          {Math.round(week.dietCompliance)}%
                        </div>
                      </div>
                    </div>
